...
throw(InvalidFormatException)
{
    ...
    wvlog << __FILE__ << ":" << __LINE__ << " - " <<"##### sti: " << hex << "0x" << sti << dec << "(" << sti << ")" << endl;
    wvlog << __FILE__ << ":" << __LINE__ << " - " <<"##### sgc: " << static_cast<int>( sgc ) << endl;
#endif
    shifterU16 >>= 4;
    istdBase = shifterU16;
#ifdef WV2_DEBUG_STYLESHEET
    wvlog << __FILE__ << ":" << __LINE__ << " - " <<"istdBase: " << hex << "0x" << istdBase << dec << "(" << istdBase << ")" << endl;
#endif
    shifterU16 = stream->readU16();
    cupx = shifterU16;
    shifterU16 >>= 4;
    istdNext = shifterU16;
#ifdef WV2_DEBUG_STYLESHEET
    wvlog << __FILE__ << ":" << __LINE__ << " - " <<"istdNext: " << hex << "0x" << istdNext << dec << "(" << istdNext << ")" << endl;
#endif
    bchUpe = stream->readU16();
#ifdef WV2_DEBUG_STYLESHEET
    wvlog << __FILE__ << ":" << __LINE__ << " - " <<"cbStd: " << cbStd << "bchUpe: " << bchUpe << endl;
#endif
...
//         wvlog << __FILE__ << ":" << __LINE__ << " - " <<"Invalid Style type detected!";
//         return false;
    }
    ...
    if (stdfSize == StdfPost2000) {
        wvlog << __FILE__ << ":" << __LINE__ << " - " <<"Warning: StdfPost2000OrNone present - skipping";
...
#ifdef WV2_DEBUG_STYLESHEET
    wvlog << __FILE__ << ":" << __LINE__ << " - " <<"stdfSize: " << stdfSize << endl;
    wvlog << __FILE__ << ":" << __LINE__ << " - " <<"start offset: " << startOffset << endl;
    wvlog << __FILE__ << ":" << __LINE__ << " - " <<"curr. position: " << stream->tell() << endl;
#endif
    // next even address
...
#ifdef WV2_DEBUG_STYLESHEET
    wvlog << __FILE__ << ":" << __LINE__ << " - " <<"new position: " << stream->tell() << endl;
    wvlog << __FILE__ << ":" << __LINE__ << " - " <<"new stdfSize: " << stdfSize_new << endl;
#endif

    if (!readStyleName( stdfSize_new, cbStd - stdfSize_new, stream )) {
        throw InvalidFormatException("Invalid Style name detected!");
//         wvlog << __FILE__ << ":" << __LINE__ << " - " <<"Invalid Style name detected!";
//         return false;
    }
...
#ifdef WV2_DEBUG_STYLESHEET
        wvlog << __FILE__ << ":" << __LINE__ << " - " <<"Adjusting the position... from " << stream->tell() - startOffset;
#endif
        stream->seek( 1, WV2_SEEK_CUR );
#ifdef WV2_DEBUG_STYLESHEET
        wvlog << __FILE__ << ":" << __LINE__ << " - " <<" to " << stream->tell() - startOffset << endl;
#endif
    }
...
#ifdef WV2_DEBUG_STYLESHEET
    wvlog << __FILE__ << ":" << __LINE__ << " - " <<"cupx: " << static_cast<int>( cupx ) << endl;
    wvlog << __FILE__ << ":" << __LINE__ << " - " <<"size: " << bytesLeft << endl;
#endif

    if (bytesLeft < 0) {
        wvlog << __FILE__ << ":" << __LINE__ << " - " <<"BUG: reading outside STD limits" << endl;
        return false;
    }
    ...
    if (!grupx) {
        ...
        wvlog << __FILE__ << ":" << __LINE__ << " - " <<"====> Error: grupx allocation!" << endl;
        return false;
    }
    ...
    for ( U8 i = 0; i < cupx; ++i) {
...
#ifdef WV2_DEBUG_STYLESHEET
        wvlog << __FILE__ << ":" << __LINE__ << " - " <<"cbUPX: " << cbUPX << endl;
#endif
        // do not overflow the allocated buffer grupx
        if (offset + cbUPX > grupxLen) {
            wvlog << __FILE__ << ":" << __LINE__ << " - " <<"====> Error: grupx would overflow!" << endl;
            return false;
        }
        for ( U16 j = 0; j < cbUPX; ++j ) {
            grupx[ offset + j ] = stream->readU8();  // read the whole UPX
#ifdef WV2_DEBUG_STYLESHEET
            wvlog << __FILE__ << ":" << __LINE__ << " - " <<"  read: " << static_cast<int>( grupx[ offset + j ] ) << endl;
#endif
        }
        ...
        if ( ( stream->tell() - startOffset ) & 1 ) {
#ifdef WV2_DEBUG_STYLESHEET
            wvlog << __FILE__ << ":" << __LINE__ << " - " <<"Adjusting the UPX position... from " << stream->tell() - startOffset;
#endif
            stream->seek( 1, WV2_SEEK_CUR );
#ifdef WV2_DEBUG_STYLESHEET
            wvlog << __FILE__ << ":" << __LINE__ << " - " <<" to " << stream->tell() - startOffset << endl;
...
{
#ifdef WV2_DEBUG_STYLESHEET
    wvlog << __FILE__ << ":" << __LINE__ << " - " <<"stdfSize:" << hex << "0x" << stdfSize << dec << "(" << stdfSize << ")" << endl;
    wvlog << __FILE__ << ":" << __LINE__ << " - " <<"stdBytesLeft:" << stdBytesLeft << endl;
#endif
    ...
    if ( stdfSize > 8 ) {
...
#ifdef WV2_DEBUG_STYLESHEET
        wvlog << __FILE__ << ":" << __LINE__ << " - " <<"length: " << length << endl;
#endif
        if ( ((length * 2) > stdBytesLeft) ) {
            wvlog << __FILE__ << ":" << __LINE__ << " - " <<"xstzName length invalid";
            return false;
        }
        ...
        if (!name) {
            wvlog << __FILE__ << ":" << __LINE__ << " - " <<"====> Error: malloc";
            return false;
        }
...
#ifdef WV2_DEBUG_STYLESHEET
            wvlog << __FILE__ << ":" << __LINE__ << " - " <<"xstzName[" << static_cast<int>( i ) << "]: " << name[i] << endl;
#endif
        }
        if ( name[ length ] != 0 ) {
            wvlog << __FILE__ << ":" << __LINE__ << " - " <<"Warning: Illegal trailing character: " <<
...
#ifdef WV2_DEBUG_STYLESHEET
        wvlog << __FILE__ << ":" << __LINE__ << " - " <<"Warning: processing on older Style Sheet";
#endif
...
#ifdef WV2_DEBUG_STYLESHEET
        wvlog << __FILE__ << ":" << __LINE__ << " - " <<"length: " << static_cast<int>( length ) << endl;
#endif
...
#ifdef WV2_DEBUG_STYLESHEET
        for ( U16 i = 0; i < length + 1; ++i )
            wvlog << __FILE__ << ":" << __LINE__ << " - " <<"xstzName[" << static_cast<int>( i ) << "]: " << static_cast<int>( name[i] ) << endl;
#endif
        if ( name[ length ] != 0 ) {
            wvlog << __FILE__ << ":" << __LINE__ << " - " <<"Warning: processing on older Style Sheet";
            wvlog << __FILE__ << ":" << __LINE__ << " - " <<"Warning: Illegal trailing character: " <<
...
#ifdef WV2_DEBUG_STYLESHEET
    wvlog << __FILE__ << ":" << __LINE__ << " - " <<"ASCII Name: '" << xstzName.ascii() << "'" << endl;
#endif
    return true;
}
    ...
    m_upechpx( 0 )
{
    ...
    if (cbStd < 0) {
        wvlog << __FILE__ << ":" << __LINE__ << " - " <<"BIG mess-up, cbStd < 0";
        m_invalid = true;
        return;
    }
...
#ifdef WV2_DEBUG_STYLESHEET
        wvlog << __FILE__ << ":" << __LINE__ << " - " <<"Empty style found: " << tableStream->tell() << endl;
#endif
        m_isEmpty = true;
        m_isWrapped = false;
        return;
    ...
    if ( tableStream->tell() != offset + cbStd ) {
        wvlog << __FILE__ << ":" << __LINE__ << " - " <<"Warning: Found a \"hole\"" << endl;
        // correct the offset
    ...
    case sgcChp:
        ...
        break;
    case sgcTbl:
        wvlog << __FILE__ << ":" << __LINE__ << " - " <<"Warning: Table Style - not supported at the moment!";
        break;
    case sgcNmbr:
        wvlog << __FILE__ << ":" << __LINE__ << " - " <<"Warning: Numbering Style - not supported at the moment!";
        break;
    default:
        wvlog << __FILE__ << ":" << __LINE__ << " - " <<"Attention: New kind of style in the stylesheet" << endl;
        break;
    }
}
...
void Style::validate(const U16 istd, const U16 rglpstd_cnt, const std::vector<Style*>& styles, U16& uds_num)
{
...
#ifdef WV2_DEBUG_STYLESHEET
    wvlog << __FILE__ << ":" << __LINE__ << " - " <<"istd: 0x" << istd << "istdBase: 0x" << hex << m_std->istdBase << endl;
#endif
    ...
    if ((m_std->istdBase != 0x0fff) &&
        ...
        wvlog << __FILE__ << ":" << __LINE__ << " - " <<"istdBase - invalid index into rglpstd!" << endl;
        return;
    }
    if (m_std->istdBase == istd) {
        wvlog << __FILE__ << ":" << __LINE__ << " - " <<"istdBase MUST NOT be same as istd!" << endl;
        return;
    }
    if ((m_std->istdBase != 0x0fff) &&
        styles[m_std->istdBase]->isEmpty()) {
        wvlog << __FILE__ << ":" << __LINE__ << " - " <<"istdBase - style definition EMPTY!" << endl;
        return;
    }
...
#ifdef WV2_DEBUG_STYLESHEET
        wvlog << __FILE__ << ":" << __LINE__ << " - " <<"Warning: istdNext - invalid index into rglpstd, setting to stiNormal!";
#endif
        m_std->istdNext = stiNormal;
    }
    if ((m_std->istdNext != 0x0fff) &&
        styles[m_std->istdNext]->isEmpty()) {
        wvlog << __FILE__ << ":" << __LINE__ << " - " <<"istdNext - style definition EMPTY!" << endl;
        return;
    }
...
#ifdef WV2_DEBUG_STYLESHEET
        wvlog << __FILE__ << ":" << __LINE__ << " - " <<"Warning: Empty xstzName detected, preparing a custom name!";
#endif
...
void Style::unwrapStyle( const StyleSheet& stylesheet, WordVersion version )
{
    ...
    wvlog << __FILE__ << ":" << __LINE__ << " - " <<"Unwrapping style:" << "ASCII Name: '" << name().ascii() << "'" << endl;
#endif
    ...
    if ( m_std->sgc == sgcPara ) {
        ...
        if ( m_std->istdBase != 0x0fff ) {
            ...
            if ( parentStyle ) {
#ifdef WV2_DEBUG_SPRMS
                wvlog << __FILE__ << ":" << __LINE__ << " - " <<"#### parent style ASCII Name: '" << parentStyle->name().ascii() << "'" << endl;
#endif
...
#ifdef WV2_DEBUG_SPRMS
            wvlog << __FILE__ << ":" << __LINE__ << " - " <<"############# Applying paragraph exceptions: " << cbUPX << endl;
#endif
            m_properties->pap().apply( data, cbUPX, parentStyle, &stylesheet, 0, version );  // try without data stream for now
#ifdef WV2_DEBUG_SPRMS
            wvlog << __FILE__ << ":" << __LINE__ << " - " <<"############# done" << "[" << name().ascii() << "]" << endl;
#endif
...
#ifdef WV2_DEBUG_SPRMS
                wvlog << __FILE__ << ":" << __LINE__ << " - " <<"############# Applying character exceptions: " << cbUPX << endl;
#endif
...
#ifdef WV2_DEBUG_SPRMS
                wvlog << __FILE__ << ":" << __LINE__ << " - " <<"our istd = " << m_upechpx->istd << " sti = " << m_std->sti << endl;
#endif
                mergeUpechpx( parentStyle, version );
            }
            else {
#ifdef WV2_DEBUG_SPRMS
                wvlog << __FILE__ << ":" << __LINE__ << " - " <<"################# NO parent style for this character style found" << endl;
#endif
            }
        }
        else {
        ...
        wvlog << __FILE__ << ":" << __LINE__ << " - " <<"Warning: Unknown style type code detected" << endl;
    }
    m_isWrapped = false;
}
...
{
    if ( !m_properties ) {
        wvlog << __FILE__ << ":" << __LINE__ << " - " <<"You requested the PAP of a character style? Hmm..." << endl;
...
const Word97::CHP& Style::chp() const
{
    if ( !m_chp ) {
        if ( !m_isEmpty ) {
            wvlog << __FILE__ << ":" << __LINE__ << " - " <<"You requested the CHP of an unknown style type? Hmm..." << endl;
            wvlog << __FILE__ << ":" << __LINE__ << " - " <<"sti == " << m_std->sti << endl;
        } else {
            wvlog << __FILE__ << ":" << __LINE__ << " - " <<"You requested the CHP of an empty style slot? Hmm..." << endl;
        }
...
{
    if ( !m_upechpx ) {
        wvlog << __FILE__ << ":" << __LINE__ << " - " <<"You requested the CHPX of a paragraph style? Hmm..." << endl;
...
namespace {
    ...
    void analyzeGrpprl( U8* grpprl, U16 count, std::vector<SprmEntry>& entries, WordVersion version )
    {
        ...
        while ( offset < count ) {
            ...
            if ( version == Word8 ) {
                sprm = readU16( grpprl );
#ifdef WV2_DEBUG_SPRMS
                wvlog << __FILE__ << ":" << __LINE__ << " - " <<"####### offset: " << offset << " sprm: 0x" << hex << sprm << dec << endl;
#endif
                grpprl += sizeof( U16 );
            }
            else {
                sprm = *grpprl++;
#ifdef WV2_DEBUG_SPRMS
                wvlog << __FILE__ << ":" << __LINE__ << " - " <<"####### offset: " << offset << " sprm (Word6/7): " << sprm << endl;
#endif
            }
    ...
    U16 copySprm( U8* destGrpprl, U8* srcGrpprl, const SprmEntry& entry, WordVersion version )
    {
        ...
        if ( version == Word8 ) {
...
#ifdef WV2_DEBUG_SPRMS
            wvlog << __FILE__ << ":" << __LINE__ << " - " <<"####### Copying sprm 0x" << hex << sprm << dec << endl;
#endif
            ...
            sprm = *srcGrpprl++;
#ifdef WV2_DEBUG_SPRMS
            wvlog << __FILE__ << ":" << __LINE__ << " - " <<"####### Copying sprm (Word6/7) " << sprm << endl;
#endif
        }

        if ( sprm != entry.sprm )
            wvlog << __FILE__ << ":" << __LINE__ << " - " <<"Ooops, we messed up the CHPX merging!" << endl;
    ...
    : m_udsNum(0)
{
    ...
    //STSH = StyleSheet
#ifdef WV2_DEBUG_STYLESHEET
    wvlog << __FILE__ << ":" << __LINE__ << " - " <<"StyleSheet::StyleSheet(): fcStshf=" << fcStshf << " lcbStshf=" << lcbStshf
    ...
    if (!cbStshi) {
        ...
        wvlog << __FILE__ << ":" << __LINE__ << " - " <<"Detected a different STSHI, check this (trying to read Word97 one)" << endl;
        m_stsh.read( tableStream, false );
    }
    ...
    if ( tableStream->tell() != static_cast<int>( fcStshf + cbStshi + 2 ) ) {
        wvlog << __FILE__ << ":" << __LINE__ << " - " <<"Warning: STSHI too big? New version?"
    ...
    //
#ifdef WV2_DEBUG_STYLESHEET
    wvlog << __FILE__ << ":" << __LINE__ << " - " <<"Reading in" << m_stsh.cstd << " styles." << endl;
#endif
    for( U16 i = 0; i < m_stsh.cstd; ++i ) {
        ...
        if (m_styles[i]->isInvalid()) {
            wvlog << __FILE__ << ":" << __LINE__ << " - " <<"==> style [" << i << "] : INVALID";
        } else {
            wvlog << __FILE__ << ":" << __LINE__ << " - " <<"==> style [" << i << "] : VALID";
        }
#endif
    }
    //styles validation
    if (m_styles.size() != m_stsh.cstd) {
        wvlog << __FILE__ << ":" << __LINE__ << " - " <<"Error: m_styles.size() != m_stsh.cstd";
    }
    for( U16 i = 0; i < m_stsh.cstd; ++i ) {
        Q_ASSERT(m_styles[i]);
...
#ifdef WV2_DEBUG_STYLESHEET
    wvlog << __FILE__ << ":" << __LINE__ << " - " <<"Done reading the stylesheet: " <<
             " stream postion:" << tableStream->tell() <<
             " expected:" << fcStshf + lcbStshf << endl;
#endif
    if ( tableStream->tell() < static_cast<int>( fcStshf + lcbStshf ) ) {
        wvlog << __FILE__ << ":" << __LINE__ << " - " <<"Warning: Didn't read all bytes of the stylesheet..." << endl;
    }
    else if ( tableStream->tell() > static_cast<int>( fcStshf + lcbStshf ) ) {
        wvlog << __FILE__ << ":" << __LINE__ << " - " <<"BUG: Read past the stylesheet area!" << endl;
    }

#ifdef WV2_DEBUG_STYLESHEET
    wvlog << __FILE__ << ":" << __LINE__ << " - " <<"##### Starting to unwrap styles" << endl;
    int i = 0;
#endif
    ...
    {
#ifdef WV2_DEBUG_STYLESHEET
        wvlog << __FILE__ << ":" << __LINE__ << " - " <<"Going to unwrap style:" << "[" << i << "]" <<
...
#ifdef WV2_DEBUG_STYLESHEET
    wvlog << __FILE__ << ":" << __LINE__ << " - " <<"##### Styles unwrapped" << endl;
#endif
    tableStream->pop();
}
