                  ...
                  OLEStreamReader* tableStream, WordVersion version )
{
    ...
    wvlog << __FILE__ << ":" << __LINE__ << " - " <<"ccpHdd=" << ccpHdd << "fc=" << fcPlcfhdd << " lcb=" << lcbPlcfhdd << endl;
    wvlog << __FILE__ << ":" << __LINE__ << " - " <<"num. of stories in PlcfHdd:" << n << endl;
    if ( version == Word8 ) {
        wvlog << __FILE__ << ":" << __LINE__ << " - " <<"num. of header/footer stories:" << n - 6 << endl;
    }
#endif
    if ( lcbPlcfhdd % sizeof( U32 ) ) {
        wvlog << __FILE__ << ":" << __LINE__ << " - " <<"Bug: m_fib.lcbPlcfhdd % 4 != 0!" << endl;
    }
    else if ( (version == Word8) && (n % headerTypes) ) {
        wvlog << __FILE__ << ":" << __LINE__ << " - " <<"Bug: #headers % " << headerTypes << " != 0!" << endl;
    }
    ...
    if ( version == Word8 ) {
        for (uint i = 0, l = 0; i < plcfsed.count(); i++) {
            wvlog << __FILE__ << ":" << __LINE__ << " - " <<"Section" << i + 1;
            wvlog << __FILE__ << ":" << __LINE__ << " - " <<"-------------------------------";
            for (uint j = 0; j < headerTypes; j++, l++) {
                wvlog << __FILE__ << ":" << __LINE__ << " - " <<strsCPs[l];
            }
        }
    } else {
        for (int i = 0; i < strsCPs.size() - 2; i++) {
            wvlog << __FILE__ << ":" << __LINE__ << " - " <<strsCPs[i];
        }
    }
    wvlog << __FILE__ << ":" << __LINE__ << " - " <<"-------------------------------";
    wvlog << __FILE__ << ":" << __LINE__ << " - " <<"second-to-last:" << strsCPs.at(strsCPs.size() - 2);
    wvlog << __FILE__ << ":" << __LINE__ << " - " <<"last:" << strsCPs.last();
#endif
...
QList<bool> Headers::headersMask( void )
{
    ...
    for (U32 i = 0; i < (U32) m_headers.size(); i++) {
        ...
        wvlog << __FILE__ << ":" << __LINE__ << " - " <<"m_headers: " << m_headers[i];
    }
#endif
    ...
    for (U32 i = 0; i < (U32) mask.size(); i++) {
        wvlog << __FILE__ << ":" << __LINE__ << " - " <<"Section" << i << ": new header/footer content: " << mask[i];
    }
#endif
    return mask;
}
