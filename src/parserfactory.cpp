...
namespace
{
    ...
    SharedPtr<Parser> setupParser( OLEStorage* storage )
    {
        ...
        if ( magic != 0xa5ec && magic != 0xa5dc )
            wvlog << __FILE__ << ":" << __LINE__ << " - " <<"+++ Attention: Strange magic number: " << magic << endl;

        U16 nFib = wordDocument->readU16();
        wvlog << __FILE__ << ":" << __LINE__ << " - " <<"nFib = 0x" << hex << nFib << dec << "(" << nFib << ")" << endl;
        ...
        // (0x0065)
            ...
            wvlog << __FILE__ << ":" << __LINE__ << " - " <<"Word-6 document found" << endl;
            return new Parser95( storage, wordDocument );
        }
        // (0x0067, 0x0068)
        else if ( nFib == 103 || nFib == 104 ) {
            wvlog << __FILE__ << ":" << __LINE__ << " - " <<"Word-7 (aka Word 95) document found" << endl;
            return new Parser95( storage, wordDocument );
        }
        // (0x00c1)
        else if ( (nFib == Word8nFib) || (nFib == 0x00c0) || (nFib == 0x00c2) )
        {
            wvlog << __FILE__ << ":" << __LINE__ << " - " <<"Word-8 (aka Word 97) document found" << endl;
            return new Parser97( storage, wordDocument );
        }
        else {
            // (0x00d9)
            if ( nFib == 217 ) {
                wvlog << __FILE__ << ":" << __LINE__ << " - " <<"Looks like a document created by Word-9/Office 2000,"
                      << "trying with the Word-8 parser." << endl;
            }
            // (0x0101)
            else if ( nFib == 257 ) {
                wvlog << __FILE__ << ":" << __LINE__ << " - " <<"Looks like a document created by Word-10/Office XP,"
                      << "trying with the Word-8 parser." << endl;
            }
            // (0x010c)
            else if ( nFib == 268 ) {
                wvlog << __FILE__ << ":" << __LINE__ << " - " <<"Looks like a document created by Word-11/Office 2003,"
                      << "trying with the Word-8 parser." << endl;
            }
            // (0x0112), ...
            else {
                wvlog << __FILE__ << ":" << __LINE__ << " - " <<"A document > Word-8 found,"
                      << "trying with the Word-8 parser." << endl;
            }
            return new Parser97( storage, wordDocument );
        }
