...
namespace wvWare
{
    ...
    {
        wvlog << __FILE__ << ":" << __LINE__ << " - " <<"STTBF::dumpStrings(): count=" << count() << " extraDataLength="
        ...
        for ( ; it != end; ++it )
            wvlog << __FILE__ << ":" << __LINE__ << " - " <<"   '" << ( *it ).ascii() << "'" << endl;
    }
    ...
    U16 STTBF::readU16( OLEStreamReader* reader, const U8** ptr ) const
    {
        if ( reader )
            ...
            *ptr += 2;
            ...
            wvlog << __FILE__ << ":" << __LINE__ << " - " <<"Warning: Internal error while reading STTBF" << endl;
            return 0;
        }
    }
