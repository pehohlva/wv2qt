 ...
 * Author: dev
 * 
 * Created on 14. dicembre 2013, 12:50
 */

#include "qtwv2.h"

#include "msdoc.h"
#include "word97_generated.h"  /// isShdNil cvAuto isShdAuto 
...
QString Conversion::color(int number, int defaultcolor, bool defaultWhite)
{
    ...
    default:
        DCALLIGRA() << " unknown color:" << number;
...
int Conversion::fillPatternStyle(int ipat)
{
    ...
    default:
        DCALLIGRA() << "Unhandled undocumented SHD ipat value: " << ipat;
        return Qt::NoBrush;
    }
}
...
QString Conversion::computeAutoColor(const wvWare::Word97::SHD& shd, const QString& bgColor, const QString& fontColor)
{
    ...
    /// http://code.woboq.org/kde/calligra/filters/words/msword-odf/wv2/src/word97_generated.cpp.html
     /// https://github.com/pehohlva/wv2qt/blob/master/src/word97_generated.h 
...
int Conversion::ditheringToGray(const quint16 ipat, bool* ok)
{
    *ok = true; // optimistic ;)
    ...
    case ipatCross:
    case ipatDiagCross:
        DCALLIGRA() << "Unsupported shading pattern (0x" << hex << ipat << ")";
        return (255 - qRound(0.3 * 255));
...
{
    DCALLIGRA() << "brc.brcType      = " << brc.brcType;
    DCALLIGRA() << "brc.dptLineWidth = " << brc.dptLineWidth;
    DCALLIGRA() << "brc.cv           = " << brc.cv;
...
int Conversion::headerMaskToHType(unsigned char mask)
{
    ...
    //DCALLIGRA() <<" hasEvenOdd=" << hasEvenOdd;
    if (hasFirst)
...
int Conversion::fldToFieldType(const wvWare::FLD* fld)
{
    ...
    if (m_fieldType < 0)
        DCALLIGRA() << "unhandled field: fld.ftl:" << (int)fld->flt;
 ...
 */
bool Conversion::readStreamPole(POLE::Storage& storage, const char* streampath, QBuffer & buffer) {
    std::string path(streampath);
    POLE::Stream stream(&storage, path);
    if (stream.fail()) {
        kWarning() << "Unable to construct " << streampath << "stream";
        return false;
    }
    ...
    if (r != stream.size()) {
        kWarning() << "Error while reading from " << streampath << "stream";
        return false;
    }
