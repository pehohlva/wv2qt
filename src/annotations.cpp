        ...
        m_annotationRef( 0 ), m_annotationRefIt( 0 )
{
    ...
    wvlog << __FILE__ << ":" << __LINE__ << " - " <<endl << "footnotes" << endl
...
AnnotationData Annotations::annotation( U32 globalCP, bool& ok )
{
         ...
         m_annotationTxtIt != m_annotationTxt.end() )
    {
        ...
        wvlog << __FILE__ << ":" << __LINE__ << " - " <<"lim:" << lim << endl;
#endif
        return AnnotationData( start, lim );
    } else {
        ok = false;
        wvlog << __FILE__ << ":" << __LINE__ << " - " <<"Bug: There is no annotation with the CP " << globalCP << endl;
...
{
    wvlog << __FILE__ << ":" << __LINE__ << " - " <<"Annotations::nextAnnotation()" << endl;
...
#ifdef WV2_DEBUG_ANNOTATIONS
    wvlog << __FILE__ << ":" << __LINE__ << " - " <<"[ PlcfandRef ]" << endl;
    ( *ref )->dumpCPs();
#endif
...
#ifdef WV2_DEBUG_ANNOTATIONS
        wvlog << __FILE__ << ":" << __LINE__ << " - " <<"[ PlcfandTxt ]" << endl;
#endif
        for ( U32 i = 0; i < lcbTxt; i += sizeof( U32 ) ) {
            txt.push_back( tableStream->readU32() );
#ifdef WV2_DEBUG_ANNOTATIONS
            wvlog << __FILE__ << ":" << __LINE__ << " - " <<"CP: " << txt.back() << endl;
#endif
        }
        txtIt = txt.begin();
    }
...
#ifdef WV2_DEBUG_ANNOTATIONS
    wvlog << __FILE__ << ":" << __LINE__ << " - " <<"Annotation init done" << endl;
#endif
}
