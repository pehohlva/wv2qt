    ...
    m_start(0), m_startIt(0), m_end(0), m_endIt(0), m_nFib(fib.nFib)
{
...
#ifdef WV2_DEBUG_BOOKMARK
        wvlog << __FILE__ << ":" << __LINE__ << " - " <<"Num. of bookmarks to start: " << m_start->count() << endl;
        wvlog << __FILE__ << ":" << __LINE__ << " - " <<"m_start init done" << endl;
        m_start->dumpCPs();
#endif
    }
...
#ifdef WV2_DEBUG_BOOKMARK
        wvlog << __FILE__ << ":" << __LINE__ << " - " <<"Num. of bookmarks to end: " << count << endl;
        wvlog << __FILE__ << ":" << __LINE__ << " - " <<"m_end/m_endCP init done" << endl;

        if ( fib.nFib < Word8nFib ) {
            m_end->dumpCPs();
        } else {
            for ( uint i = 0; i < m_endCP.size(); i++ ) {
                wvlog << __FILE__ << ":" << __LINE__ << " - " <<"dumpCPs:   " << m_endCP[i] << endl;
            }
        }
#endif
    }
...
#ifdef WV2_DEBUG_BOOKMARK
    wvlog << __FILE__ << ":" << __LINE__ << " - " <<"Bookmark init done" << endl;
#endif
    ...
    if (!valid(num, fib.ccpText)) {
        wvlog << __FILE__ << ":" << __LINE__ << " - " <<"Num. of invalid bookmarks:" << num;
    }
...
{
#ifdef WV2_DEBUG_BOOKMARK
    wvlog << __FILE__ << ":" << __LINE__ << " - " <<" globalCP=" << globalCP << endl;
#endif
    ok = false;
    ...
    {
        if (m_valid.isEmpty()) {
            wvlog << __FILE__ << ":" << __LINE__ << " - " <<"BUG: m_valid empty?";
...
#ifdef WV2_DEBUG_BOOKMARK
        wvlog << __FILE__ << ":" << __LINE__ << " - " <<"start = " << start << endl;
        wvlog << __FILE__ << ":" << __LINE__ << " - " <<"end = " << end << endl;
        wvlog << __FILE__ << ":" << __LINE__ << " - " <<"name = " << name.ascii() << endl;
        wvlog << __FILE__ << ":" << __LINE__ << " - " <<"valid = " << ok << endl;
#endif
...
#ifdef WV2_DEBUG_BOOKMARK
            wvlog << __FILE__ << ":" << __LINE__ << " - " <<"Warning: Skipped invalid bookmark!";
#endif
        }
    }
    return ret;
...
#ifdef WV2_DEBUG_BOOKMARK
        wvlog << __FILE__ << ":" << __LINE__ << " - " <<"Bookmark skipped! CP:" << globalCP;
#endif
    }
}
    ...
    if (m_nFib < Word8nFib) {
        ...
        while (startIt.current()) {
                 ...
                 (startIt.currentStart() > ccpText) )
            {
                ...
                num++;
#ifdef WV2_DEBUG_BOOKMARK
                wvlog << __FILE__ << ":" << __LINE__ << " - " <<"bkmk" << n << ": (startCP > endCP) || endCP missing";
#endif
            } else {
                m_valid.append(true);
            }
        ...
        while (startIt.current()) {
            ...
            if (ibkls.contains(ibkl) || (ibkl > m_endCP.size())) {
                ...
                num++;
#ifdef WV2_DEBUG_BOOKMARK
                wvlog << __FILE__ << ":" << __LINE__ << " - " <<"bkmk" << n << ": ibkl invalid!";
                n++;
#endif
                ++startIt;
                continue;
                 ...
                 (startIt.currentStart() > ccpText) )
            {
                ...
                num++;
#ifdef WV2_DEBUG_BOOKMARK
        wvlog << __FILE__ << ":" << __LINE__ << " - " <<"bkmk" << n << ": startCP > endCP (" <<
...
#ifdef WV2_DEBUG_BOOKMARK
        wvlog << __FILE__ << ":" << __LINE__ << " - " <<"Warning: bookmark names missing!  Using custom names.";
        wvlog << __FILE__ << ":" << __LINE__ << " - " <<"Num. of bookmark names:" << m_name.size();

        std::vector<UString>::const_iterator it = m_name.begin();
        while(it != m_name.end()) {
            wvlog << __FILE__ << ":" << __LINE__ << " - " <<"bkmk name:" << (*it).ascii();
            ++it;
        }
#endif
    }
